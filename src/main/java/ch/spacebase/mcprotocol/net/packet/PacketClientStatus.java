...
public class PacketClientStatus extends Packet {
	...
	public void handleServer(ServerConnection conn) {
		if(this.status == 0 && conn.getServer().verifyUsers()) {
			String encrypted = new BigInteger(Util.encrypt(conn.getLoginKey(), conn.getServer().getKeys().getPublic(), conn.getSecretKey())).toString(16);
		...
		LoginEvent event = conn.call(new LoginEvent());
		if(event.disallow()) {
			conn.disconnect(event.getReason());
			return;
		}
		...
		conn.call(new LoginFinishEvent());
	}
